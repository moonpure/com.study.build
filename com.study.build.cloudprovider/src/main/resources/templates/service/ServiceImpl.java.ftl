...
<Boolean> save(${entityName} entity) {
    ...
    <Boolean> updateById(${entityName} entity) {
        ...
        <Boolean> update(List<${entityName}> entities) {
            ...
            boolean bool = db${entityName}Service.update(entities.get(0), new QueryWrapper<${entityName}
            >(entities.get(1)));
            if (!bool) {
            return Result.createFailure(ResultCode.DATABASE_EDIT_FAILURE);
            }
            ...
            <T> Result
                <List
                <${entityName}>> selectIn(RequestIn<${entityName},T> entity)
                {
                ...
                public Result
                <List
                <${entityName}>> selectBetween(RequestBetween<${entityName}> entity) {
                if (entity == null) {
                return Result.createFailure(ResultCode.PARAM_IS_INVALID);
                }
                Page<${entityName}> pageEntity = new Page<>();
                ...
                List
                <BetweenBase
                <Long>> longList = entity.getBetweenLong();
                    if (longList != null && longList.size() > 0) {
                    for (BetweenBase
                    <Long> beLong : longList) {
                        if (beLong == null) {
                        continue;
                        }
                        ...
                        List
                        <BetweenBase
                        <LocalDateTime>> dateList = entity.getBetweenDate();
                            if (dateList != null && dateList.size() > 0) {
                            for (BetweenBase
                            <LocalDateTime> beDate : dateList) {
                                if (beDate == null) {
                                continue;
                                }
                                ...
                                <T> Result
                                    <List
                                    <${entityName}>> selectQuery(RequestQuery<${entityName}, T> entity) {
                                    if (entity == null) {
                                    return Result.createFailure(ResultCode.PARAM_IS_INVALID);
                                    }
                                        ...
                                        <Long> beLong : longList) {
                                            ...
                                            <LocalDateTime>> dateList = entity.getBetweenDate();
                                                ...
                                                <LocalDateTime> beDate : dateList) {
                                                    ...
                                                    if (!StringUtils.isAllBlank(entity.getColumnName()) &&
                                                    entity.getInValues() != null && entity.getInValues().size() > 0) {
                                                    query.in(entity.getColumnName(), entity.getInValues());
                                                    }

                                                    IPage<${entityName}> selectPage = db${entityName}
                                                    Service.page(pageEntity, query);
                                                    return Result.createBySuccess(selectPage.getRecords());
                                                    }
                                                    }
